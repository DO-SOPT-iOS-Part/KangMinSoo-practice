...
class ViewController: UIViewController {
    ...
    @objc private func registerButtonTap() {
        Task {
            do {
                let status = try await CheckUserNameService.shared.PostRegisterData(userName: self.usreName)
                if status == false {
                    ...
                    let alert = UIAlertController(title: "중복된 유저아이디로 인한 실패", message: "흑흑", preferredStyle: UIAlertController.Style.alert)
                    let okAction =  UIAlertAction(title: "확인", style: UIAlertAction.Style.default)
                    alert.addAction(okAction)
                    self.present(alert, animated: true)
                }
